...
"""
Analytics Aggregation DAG — агрегація даних з операційної схеми в аналітичну.
Запускається раз на день о 2:00 ночі.
"""
...
async def _aggregate_fact_vacancies(target_date: date) -> int:
    """Агрегувати вакансії з operational в fact_vacancies."""
...
async def _aggregate_skills_daily(target_date: date) -> int:
    """Агрегувати статистику скілів за день."""
...
async def _aggregate_salary_by_skill() -> int:
    """Агрегувати зарплатну статистику по скілах."""
    from sqlalchemy import func, select, text
...
def analytics_aggregation():
    @task
    def aggregate_fact_vacancies() -> int:
        target_date = date.today()
        return asyncio.run(_aggregate_fact_vacancies(target_date))

    @task
    def aggregate_skills_daily() -> int:
        target_date = date.today()
        return asyncio.run(_aggregate_skills_daily(target_date))

    @task
    def aggregate_salary_by_skill() -> int:
        return asyncio.run(_aggregate_salary_by_skill())
